locals {
  ...
  name                  = format("%s-aurora", local.name_prefix)
  tags = merge({
  ...
  master_password       = var.create_cluster && var.password == null && var.is_primary_cluster ? random_password.master_password[0].result : var.password
...
resource "random_password" "master_password" {
  count = var.create_cluster && var.password == null ? 1 : 0
...
resource "aws_rds_cluster" "this" {
  ...
  final_snapshot_identifier           = "${var.final_snapshot_identifier_prefix}-${local.name}-${element(concat(random_id.snapshot_identifier.*.hex, [""]), 0)}"
...
resource "aws_rds_cluster_instance" "this" {
  ...
  identifier                      = try(lookup(var.instances_parameters[count.index], "instance_name"), "${local.name}-${count.index + 1}")
...
resource "aws_iam_role" "rds_enhanced_monitoring" {
  ...
  name                  = format("%sRDSEnhancedMonitoringRole", var.context.project)
...
resource "aws_security_group" "this" {
  ...
  name          = "${local.name}-sg"
  vpc_id        = var.vpc_id
  description   = var.security_group_description == "" ? "Control traffic to/from RDS Aurora ${local.name}-sg" : var.security_group_description
  tags          = merge({
                      Project     = var.context.project
                      Environment = var.context.environment
                      Team        = var.context.team
